...
```bash
npx wrangler d1 create smartwallet-db
```
...
(Hintergrund: `wrangler d1 execute smartwallet-db --remote --file=./schema.sql`.)
...
```
https://smartwallet.<dein-subdomain>.workers.dev
```
...
1. `https://smartwallet.<dein-subdomain>.workers.dev/register` öffnen
...
npx wrangler d1 execute smartwallet-db --remote --file=./reset.sql
rm reset.sql
```
...
1. `https://smartwallet.<dein-subdomain>.workers.dev` öffnen → mit einem der neuen Nutzer einloggen
...
```bash
# DB erreichbar?
curl https://smartwallet.<dein-subdomain>.workers.dev/api/health
# → {"status":"ok","db":"connected"}

# Login funktioniert? (Cookie-Jar lokal speichern)
curl -c cookies.txt -X POST https://smartwallet.<dein-subdomain>.workers.dev/api/login \
  -H 'Content-Type: application/json' \
  -d '{"email":"anna@beispiel.de","password":"EinLangesSicheresPasswort"}'

# Dashboard-Export als Gegenprobe (Monat im Format YYYY-MM)
curl -b cookies.txt "https://smartwallet.<dein-subdomain>.workers.dev/api/transactions?month=2026-09"
rm cookies.txt
```
...
- **Schema-Änderungen** (neue Spalten/Tabellen): SQL-Datei anlegen und
  `npx wrangler d1 execute smartwallet-db --remote --file=./datei.sql` ausführen.
...
Wenn die App unter `smartwallet.beispiel.de` statt `*.workers.dev` laufen soll
(du brauchst eine bei Cloudflare gehostete Zone/Domain):

```toml
# wrangler.toml ergänzen
routes = [
  { pattern = "smartwallet.beispiel.de", custom_domain = true }
]
```
...
| Registrierung schlug früher ab (vor v0.3-Fix) und es gibt leere Haushalte | Aufräumen: `npx wrangler d1 execute smartwallet-db --remote --command "DELETE FROM households WHERE id NOT IN (SELECT DISTINCT household_id FROM users)" -y` |
...
| Login immer „E-Mail oder Passwort ist falsch" | Nutzer existiert nicht in der **remote** D1 → Schritt 6 prüfen (`wrangler d1 execute smartwallet-db --remote --command "SELECT id, email FROM users"`) |
